...
// bitola). NPS "5"/DN125 — bitola criada pelo seed (ver TRIM_SEED_CRIAR_BITOLA), padrão
// V-port Kvs250 (decisão do usuário).
const ARI_12440_TRIMS_2013_08 = {
  ...
  '5': [{ plug: 'parabolico', kvs: 250, dpmax: 1, sede: 126, curso: 50 }, { plug: 'parabolico', kvs: 160, dpmax: 1, sede: 126, curso: 50 }, { plug: 'vport', kvs: 250, dpmax: 10, sede: 126, curso: 50 }],
...
const ARI_12440_F2F = { '1/2': 130, '3/4': 150, '1': 160, '1.1/4': 180, '1.1/2': 200, '2': 230, '2.1/2': 290, '3': 310, '4': 350, '5': 400, '6': 480, '8': 600, '10': 730 };
...
// Bitolas que o seed pode CRIAR do zero quando ainda não existem em VALV[modelo].sizes —
// espelho exato de ARI_TRIM_SEED_CRIAR_BITOLA em index.html (ver comentário completo lá).
const TRIM_SEED_CRIAR_BITOLA = {
  '12440': { '5': { plug: 'vport', kvs: 250 } },
};
...
function applyTrimSeed(valvIn) {
  ...
  relevantes.forEach((mdl) => {
    ...
    // clona só o necessário do modelo (trims/f2f/sizes/apas/curso por bitola); o resto
    // (PMax_barg, internos, xt...) fica por referência. sizes/apas/curso só precisam de clone
    // porque TRIM_SEED_CRIAR_BITOLA pode ESCREVER neles (criar bitola nova) — sem clonar, essa
    // escrita mutaria mdOrig.sizes/apas/curso, que pode ser o objeto compartilhado de
    // lib/engine.js (mesmo cuidado já aplicado a trims/f2f).
    ...
    md.sizes = Object.assign({}, mdOrig.sizes);
    md.apas = Object.assign({}, mdOrig.apas);
    md.curso = Object.assign({}, mdOrig.curso);
    if (f2fSeed) Object.keys(f2fSeed).forEach((dn) => {
      if (md.f2f[dn] == null || md.f2f[dn] === '') md.f2f[dn] = f2fSeed[dn];
    });
    const criarCfg = TRIM_SEED_CRIAR_BITOLA[mdl] || {};
    Object.keys(seed).forEach((dn) => {
      const existentes = md.trims[dn] = md.trims[dn] || [];
      const jaTemPadrao = existentes.some((r) => r && r.padrao);
      // Bitola aprovada para criação e ainda ausente do cadastro: define o Cv AQUI, antes de
      // calcular atualKvs abaixo — o match por tolerância padrão (mesmo código de sempre) já
      // marca sozinho o trim certo como padrão, sem precisar de uma segunda lógica especial.
      let bitolaCriadaAgora = false;
      if (md.sizes[dn] == null && criarCfg[dn] != null) {
        md.sizes[dn] = criarCfg[dn].kvs / 0.865;
        bitolaCriadaAgora = true;
      }
      ...
      // Bitola recém-criada: deriva apas/curso (nível bitola) do trim padrão que acabou de ser
      // marcado — mesmo padrão do admDeriveFromTrims em index.html (só preenche o que ainda
      // estiver vazio; ex. curso['5'] do 12440 já existia órfão com o valor certo).
      if (bitolaCriadaAgora) {
        const pad = existentes.find((r) => r && r.padrao);
        if (pad) {
          if (md.apas[dn] == null && pad.sede != null) { const area = seatAreaM2Local(pad.sede); if (area != null) md.apas[dn] = area; }
          if (md.curso[dn] == null && pad.curso != null) md.curso[dn] = pad.curso;
        }
      }
    });
  });
...
// Área de passagem (m²) a partir do Ø de sede (mm) — cópia mínima de seatAreaM2 (index.html,
// admin), só para derivar VALV.apas de uma bitola recém-criada pelo seed (mesma unidade da
// matriz "Área de passagem" existente).
function seatAreaM2Local(sedeMm) {
  const d = Number(sedeMm);
  if (!isFinite(d) || d <= 0) return null;
  return Math.PI / 4 * Math.pow(d / 1000, 2);
}
